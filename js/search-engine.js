...
(function () {
    ...
    function getCountriesList() {
        const db = (typeof window !== 'undefined' && window.VISION_VISA_COUNTRIES) || {};
    ...
    function initSearchEngine() {
        ...
        if (destInput.dataset.searchBound === 'true') {
            return;
        }
        destInput.dataset.searchBound = 'true';
        ...
        function renderDestDropdown(query) {
            ...
            let matches = countriesList.filter(c => {
                const matchesName = c.name.toLowerCase().includes(trimmed) || c.slug.toLowerCase().includes(trimmed);
            ...
            // Priority: startsWith comes first
            matches.sort((a, b) => {
            ...
            if (matches.length === 0) {
                destDropdown.innerHTML = `<div class="search-dropdown-empty">No destinations found</div>`;
        ...
        if (visaSelect) {
            visaSelect.addEventListener('change', () => {
                if (destDropdown.classList.contains('show') || destInput.value.trim()) {
                    renderDestDropdown(destInput.value);
                }
        ...
        function filterHomepageCards(visaType) {
            ...
            cards.forEach(card => {
                ...
                if (match && match[1]) {
                    ...
                    if (countryData && countryData.visaType && countryData.visaType.toLowerCase().includes(selectedVisa)) {
                        card.style.display = '';
                    } else {
        ...
        if (natInput && natDropdown && !natInput.dataset.natBound) {
            natInput.dataset.natBound = 'true';
        ...
        function executeSearch() {
            ...
            if (!targetSlug) {
                ...
                if (match) {
                    ...
                    // StartsWith priority
                    const startsMatch = countriesList.find(c => c.name.toLowerCase().startsWith(query));
                    if (startsMatch) {
            ...
            if (targetSlug) {
                ...
                if (errorMsg) {
                    ...
                    if (msgSpan) msgSpan.textContent = 'No matching destination found. Please select from the dropdown.';
                    errorMsg.style.display = 'flex';
                }
            }
        }
    ...
    if (typeof window !== 'undefined') {
        window.VISION_VISA_INIT_SEARCH = initSearchEngine;
    }
